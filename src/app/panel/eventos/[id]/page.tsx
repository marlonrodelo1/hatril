...
export default async function EventoPage({
  ...
  return (
    <>
      {/*
       * En el subtítulo va SOLO la fecha, y el lugar y el precio bajan a una
       * tarjeta.
       *
       * El subtítulo de la cabecera es una línea que se recorta, y de los tres
       * datos el lugar es justo el que se perdería: la fecha con año ya ocupa
       * casi cuarenta caracteres. La fecha cabe siempre y es lo que se quiere
       * tener delante mientras se baja por la lista de inscritos, porque la
       * cabecera es `sticky`.
       */}
      <CabeceraPanel
        titulo={e.titulo}
        subtitulo={formatearInstante(e.inicioEn, tz, { conAnio: true })}
      ...
      <Contenedor>
         ...
         * Por debajo, todo apilado: partir en dos una tableta deja dos columnas
         * estrechas y la lista de inscritos —que lleva nombre, correo y
         * teléfono en la misma fila— es la primera que deja de leerse.
         *
         * La lista va en la columna ancha porque es lo que crece: puede tener
         * doscientas filas. Lo demás —los cuatro botones de estado y el bloque
         * de borrar— ocupa lo que ocupa y no cambia de tamaño.
         *
        ...
        <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <section className="flex min-w-0 flex-col gap-4">
            <div className="flex flex-wrap items-end justify-between gap-3">
              ...
              {/* El CSV se queda junto a la lista que exporta, y no con los
                  botones de estado de la otra columna: solo existe cuando hay
                  filas, y al lado del recuento se entiende sin leer nada. */}
              {lista.length > 0 && (
                <Button
                  variant="outline"
                  size="sm"
                      ...
                      <div className="flex flex-none items-center gap-2">
                        {e.precio && (
                          <form action={marcarPagada.bind(null, i.id, id)}>
                            ...
                            {/* `secondary` y no `default`: este botón está
                                dentro del `map` de inscritos, y en un evento
                                de pago con treinta personas sin pagar eran
                                treinta naranjas en columna. El naranja de esta
                                pantalla es «Publicar en la web», que es único.
                                El verde sigue distinguiendo pagada de no
                                pagada, que es la información que da el color. */}
                            <Button
                              type="submit"
                              size="sm"
                              variant={i.pagado ? 'outline' : 'secondary'}
                            >
          ...
          {/*
           * La columna estrecha: lo que se decide sobre el evento, no sobre
           * quién viene. Ancho fijo de 360 px y no una fracción, para que los
           * botones no acaben repartidos a lo largo de un monitor de 2560.
           */}
          <aside className="flex min-w-0 flex-col gap-6">
            ...
            {(e.lugar || e.precio) && (
              <section className="flex flex-col gap-2 rounded-xl border border-border bg-surface p-5">
                {e.lugar && (
                  <span className="inline-flex items-start gap-2 text-[14px] text-foreground">
                    <MapPin
            ...
            <section className="flex flex-col gap-4 rounded-xl border border-border bg-surface p-5">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-[13px] text-muted-foreground">
                  Anunciarlo y admitir inscripciones son dos cosas distintas:
                  puedes seguir anunciando el retiro cuando ya no queden plazas.
                </p>
              </div>
              ...
              {e.publicado && !ctx.iglesia.webPublica && (
                <Aviso>
                  El evento está publicado, pero la web de tu iglesia no lo está,
                  así que nadie puede verlo. Se activa en Ajustes.
                </Aviso>
              )}
            </section>
            ...
            <section className="flex flex-col gap-4 rounded-xl border border-danger-border bg-danger-soft p-5">
              <div className="flex flex-col gap-1">
                ...
                <p className="max-w-[62ch] text-[13px] leading-relaxed text-muted-foreground">
                  Quien se apunta a un evento sin ser de la iglesia confía sus
                  datos para eso y nada más. Lo que tienes pactado en los términos
                  es no guardar la lista más de{' '}
                  <strong className="font-semibold">90 días</strong> desde que el
                  evento termina. No hay ninguna tarea automática que lo haga por
                  ti: este botón es la forma de cumplirlo.
                </p>
              </div>
